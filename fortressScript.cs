  ...
  void OnTriggerEnter(Collider col)
  {
    ...
    if (col.name == "redlaser(Clone)" || col.tag == "PlayersLaser")
    {
      ...
      if (buildingHealth <= 0)
      {
        ...
        {
          GameObject.Find("TPlayer").GetComponent<PlayerControls>().buildingsDestroyed++;
        }
        if (GameObject.Find("SPlayer") != null)
        {
          GameObject.Find("SPlayer").GetComponent<PlayerControls>().buildingsDestroyed++;
        }
        if (GameObject.Find("NPlayer") != null)
        {
          GameObject.Find("NPlayer").GetComponent<PlayerControls>().buildingsDestroyed++;
        }
        Destroy(gameObject);
      }
    }
